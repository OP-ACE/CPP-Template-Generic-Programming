...
//   Person p2(std::move(p1));   //! 调用 移动构造, 好理解
//   Person p3(p2);              //! 调用 模板构造函数, 
}
...
// page 333), for a nonconstant lvalue Person pthe member template
// template<typename STR>
// Person(STR&& n)
// is a better match than the (usually predeﬁned) copy constructor:
// Person (Person const& p)
// STRis just substituted with Person&, while for the copy constructor a conversion to constis neces-
// sary.
...
// Person(p1) doesn't work because overload resolution will erroneously prefer the constructor template over the implicit copy constructor. 
// If your question is how to make the copy constructor usable despite the converting constructor template, you should have asked for that. 
// It has nothing to do with perfect-forwarding.